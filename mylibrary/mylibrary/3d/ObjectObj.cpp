...
ObjectObj::PipelineSet ObjectObj::pipelineSet;
Camera* ObjectObj::camera = nullptr;

void ObjectObj::StaticInitialize(ID3D12Device* device, Camera* camera)
{
	// 再初期化チェック
	assert(!ObjectObj::device);
	...
	// モデルの静的初期化
	ObjLoder::StaticInitialize(device);
}

void ObjectObj::CreateGraphicsPipeline()
{
...
ObjectObj* ObjectObj::Create(ObjLoder* model)
{
	// 3Dオブジェクトのインスタンスを生成
...
bool ObjectObj::Initialize()
{
	...
	CD3DX12_RESOURCE_DESC resource = CD3DX12_RESOURCE_DESC::Buffer((sizeof(ConstBufferDataB0) + 0xff) & ~0xff);
