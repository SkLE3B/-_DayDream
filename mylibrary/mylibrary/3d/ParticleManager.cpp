...
void ParticleManager::Initialize()
{
	...
	CD3DX12_RESOURCE_DESC resource = CD3DX12_RESOURCE_DESC::Buffer((sizeof(ConstBufferData) + 0xff) & ~0xff);
...
void ParticleManager::Update()
{
	...
	// 寿命が尽きたパーティクルを全削除
	particles.remove_if([](Particle& x) { return x.current_frame >= x.num_frame; });
	...
	for (std::forward_list<Particle>::iterator it = particles.begin();
		...
		it->current_frame++;
		// 進行度を0～1の範囲に換算
		float f = (float)it->num_frame / it->current_frame;
		...
		// カラーの線形補間
		it->color = it->initial_color + (it->final_color - it->initial_color) / f;

		// スケールの線形補間
		it->scale = it->initial_scale + (it->final_color_scale - it->initial_scale) / f;

		// スケールの線形補間
		it->rotation = it->initial_rotation + (it->final_rotation - it->initial_rotation) / f;
	}
...
void ParticleManager::Add(int life, XMFLOAT3 position, XMFLOAT3 velocity, XMFLOAT3 accel, float start_scale, float end_scale)
{
	...
	p.velocity = velocity;
	p.accel = accel;
	p.initial_scale = start_scale;
	p.final_color_scale = end_scale;
	p.num_frame = life;
}
...
void ParticleManager::LoadTexture()
{
	...
	result = LoadFromWICFile(
		L"Resources/textures/honoo.png", WIC_FLAGS_NONE,
		&metadata, scratchImg);
	if (FAILED(result)) {
		assert(0);
	}
	...
	CD3DX12_HEAP_PROPERTIES heapPro = CD3DX12_HEAP_PROPERTIES(D3D12_CPU_PAGE_PROPERTY_WRITE_BACK, D3D12_MEMORY_POOL_L0);
...
void ParticleManager::CreateModel()
{
	...
	CD3DX12_HEAP_PROPERTIES heapPro = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
	CD3DX12_RESOURCE_DESC resource = CD3DX12_RESOURCE_DESC::Buffer(sizeof(VertexPos) * vertexCount);
	// 頂点バッファ生成
	result = device->CreateCommittedResource(
		&heapPro,
		D3D12_HEAP_FLAG_NONE,
