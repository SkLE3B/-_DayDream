...
/// 静的メンバ変数の実体
/// </summary>
const std::string ObjLoder::baseDirectory = "Resources/models/";
ID3D12Device* ObjLoder::device = nullptr;
UINT ObjLoder::descriptorHandleIncrementSize = 0;

void ObjLoder::StaticInitialize(ID3D12Device* device)
{
	// 再初期化チェック
	assert(!ObjLoder::device);

	ObjLoder::device = device;

	// メッシュの静的初期化
	Mesh::StaticInitialize(device);
}

ObjLoder* ObjLoder::CreateFromOBJ(const std::string& modelname)
{
	// メモリ確保
	ObjLoder* instance = new ObjLoder;
...
void ObjLoder::Initialize(const std::string& modelname)
{
...
void ObjLoder::LoadMaterial(const std::string& directoryPath, const std::string& filename)
{
	// ファイルストリーム
	std::ifstream file;
	// マテリアルファイルを開く
...
void ObjLoder::AddMaterial(Material* material)
{
	// コンテナに登録
	materials.emplace(material->name, material);
}

void ObjLoder::CreateDescriptorHeap()
{
...
void ObjLoder::Draw(ID3D12GraphicsCommandList* cmdList)
{
	// デスクリプタヒープの配列
	if (descHeap) {
