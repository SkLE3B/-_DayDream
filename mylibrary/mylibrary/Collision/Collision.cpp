...
bool Collision::IsCheckHit3D(const XMFLOAT3& position1, float r1, const XMFLOAT3& position2, float r2)
{
	float a = (position1.x - position2.x) * (position1.x - position2.x);
	float b = (position1.y - position2.y) * (position1.y - position2.y);
	float c = (position1.z - position2.z) * (position1.z - position2.z);
	float r = (r1 + r2) * (r1 + r2);
	if (a + b + c <= r)
	{
		return true;
